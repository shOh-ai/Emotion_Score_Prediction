...
folder_path = '/Users/sh_oh/Library/CloudStorage/Dropbox/Data/2023-1/BDP/ECSMP_Dataset/EDA'
...
# EDA.csv 파일 경로
eda_directory = '/Users/sh_oh/Library/CloudStorage/Dropbox/Data/2023-1/BDP/ECSMP_Dataset/EDA'
eda_files = os.listdir(eda_directory)

# 숫자.csv 파일 경로
csv_directory = '/Users/sh_oh/Library/CloudStorage/Dropbox/Data/2023-1/BDP/ECSMP_Dataset/EDA'
csv_files = os.listdir(csv_directory)

# 저장할 디렉토리 경로
output_directory = '/Users/sh_oh/Library/CloudStorage/Dropbox/Data/2023-1/BDP/ECSMP_Dataset/Output'
...
# 반복문을 사용하여 EDA.csv 파일을 각 구간에 맞게 분할하여 저장
for i in range(len(folder_names)):
    # EDA.csv 파일 경로
    eda_csv_file = os.path.join(eda_directory, folder_names[i], 'EDA.csv')

    # 숫자.csv 파일 경로
    csv_file = os.path.join(csv_directory, folder_names[i], f'{folder_names[i]}.csv')

    # 001.csv의 2열 2행부터 21행까지의 값 추출 및 정수 부분만 사용
    with open(csv_file, 'r') as file:
        csv_reader = csv.reader(file)
        column_2_values = list(csv_reader)[1:21]
        row_numbers = [math.floor(float(row[1]))+2 for row in column_2_values]

    # EDA.csv 파일을 각 구간에 맞게 분할하여 저장
    with open(eda_csv_file, 'r') as file:
        csv_reader = csv.reader(file)

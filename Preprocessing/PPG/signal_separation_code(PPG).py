...
# ## Move Calculated files for event_marker_ppg 
...
# Event_marker_ppg 폴더 경로
event_marker_ppg_folder = '/Users/sh_oh/Library/CloudStorage/Dropbox/Data/2023-1/BDP/ECSMP_Dataset/Event_marker_ppg'

# PPG 폴더 경로
ppg_folder = '/Users/sh_oh/Library/CloudStorage/Dropbox/Data/2023-1/BDP/ECSMP_Dataset/PPG'

# Event_marker_ppg 폴더 내의 모든 '.csv' 파일 가져오기
csv_files = [file for file in os.listdir(event_marker_ppg_folder) if file.endswith('.csv')]

for csv_file in csv_files:
    # '.csv' 파일의 이름으로 하위 폴더 경로 생성
    folder_name = os.path.splitext(csv_file)[0]
    folder_path = os.path.join(ppg_folder, folder_name)
    
    # 하위 폴더가 이미 존재하는 경우에만 이동
    if os.path.exists(folder_path):
        csv_file_path = os.path.join(event_marker_ppg_folder, csv_file)
